...
DisjunctionTask::DisjunctionTask(Task * s1, Task * s2)
{
...
ternary_t DisjunctionTask::getResult()
{
        assert(subTask1);
	subresult1 = subTask1->getResult();
	if(subresult1 == TERNARY_TRUE)
	{
		result = TERNARY_TRUE;
		return result;
	}
	result = subresult2 = subTask2->getResult();
	if((subresult1 == TERNARY_UNKNOWN) && (subresult2 != TERNARY_TRUE))
	{
...
Task * DisjunctionTask::buildTask()
{
	assert(false);
